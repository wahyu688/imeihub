...
function buildAdminGroupedOrders() {
  ...
  headers.forEach(header => {
    ...
    while (current && !current.classList.contains('order-group-header')) {
      const cells = current.querySelectorAll('td');
      if (cells.length < 6) break;
      const amountText = cells[5].textContent.match(/\d[\d.]*/g)?.[0]?.replace(/\./g, '');
      ...
      orders.push({
        ...
        orderDate: cells[4]?.textContent.trim(),
    ...
    if (orders.length > 0) {
      window.adminGroupedOrders[label] = orders;
    }
  });
}
